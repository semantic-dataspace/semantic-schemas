...
A **fixed-structure template** for documenting a characterization campaign.
...
4. Fill in this template  →  characterization/campaign/PMDCo/   (this schema)
5. Record detailed result →  characterization/tensile-test/TTO/
...
## How it differs from the record schemas

The `characterization/generic/` and variant schemas (e.g. `generic/PMDCo/`, `tensile-test/TTO/`)
...
results go in a `generic/` or variant schema; `step_reference` links the two together.

Use this template when you want every experiment record to be traceable. Use the
`generic/` or variant schemas when you only need the measurement data.
...
- [Tensile Test (TTO)](../tensile-test/TTO/README.md): detailed result schema
