...
```sparql
# old
FILTER(STR(?conformsTo) = "…/characterization/tensile-test/#v5.0.0")
# new
...
- **Folder moved** from `characterization/step/tensile-test/TTO/` to
  `characterization/tensile-test/TTO/` (domain-wide restructuring that removes
  the `step/` intermediate level; no field or graph changes).
- `x-schema-id` updated from `…/characterization/step/tensile-test/TTO/` to
...
```sparql
# old
FILTER(?schema = <…/characterization/step/tensile-test/TTO/#v3.0.2>)
# new
FILTER(?schema = <…/characterization/tensile-test/TTO/#v4.0.0>)
```
