...
### Why TTO is a variant schema, not a campaign schema

TTO records the **measurement result** of a single test procedure. It answers
*"what values were obtained from this specimen."*

The [`characterization/campaign/PMDCo/`](../../campaign/PMDCo/README.md) schema
serves a different role: it records **campaign provenance**, answering
*"who ran this test, on which machine, on which specimen, and which result records
it produced."* That schema links to a TTO (or PMDCo) result via `step_reference`.

TTO is one of those results. It is a measurement result schema, not a provenance
record for the campaign that produced it.
